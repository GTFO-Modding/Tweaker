...
[HarmonyPatch(typeof(EnemyCostManager), nameof(EnemyCostManager.AddCost), new Type[] { typeof(eDimensionIndex), typeof(float) })]
class EnemyCostManager_AddCost
{
    public static void Prefix(EnemyCostManager __instance, ref float points)
    {
        ...
        Log.Debug($"Adding enemy wave cost of {points} out of {__instance.m_allowedTotalCost}");
    }
}